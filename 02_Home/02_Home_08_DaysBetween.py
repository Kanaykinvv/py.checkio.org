...
# Вы должны найти разницу в днях между имеющимися датами. Например, между сегодня и вчера = 1 день. Разница между
# днями всегда будет положительной или нулем, не забывайте про абсолютное значение.
#
...
# Зачем это нужно: Python идет "в комплекте с батарейками", таким образом, вам полезно будет узнать, как использовать
# готовые модули, чтобы не изобретать велосипед снова и снова.
#
# Предусловия: Даты между 1 января 1 и 31 декабря 9999. Даты корректны.
import datetime
def days_diff(a, b):
    # your code here
    date_start = datetime.date(a[0], a[1], a[2])
    date_end = datetime.date(b[0], b[1], b[2])
    if date_end > date_start:
        minus = date_end - date_start
    else:
        minus = date_start - date_end
    return minus.days
