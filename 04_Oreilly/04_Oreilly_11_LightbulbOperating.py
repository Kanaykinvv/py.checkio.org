...
#     2. Проверяем ремя работы лампочек при поступлении сигнала
#         2.1. Если лампочка не горела:
#             2.1.1. Проверяем выработку ее ресурса - Если он остался или он бесконечен (не задан):
#                   - включаем лампочку (lights[номер_лампочки][0] = True)
#                   - запоминаем время включения (lights[номер_лампочки][1])
#             2.1.2 Проверяем выработку ее ресурса - Если ресурс задан и выработан:
#               - пропускаем включение данной лампочки
#               - прерываем текущий шаг цикла (?)
#         2.2. Если лампочка горела:
    #           2.2.1.
