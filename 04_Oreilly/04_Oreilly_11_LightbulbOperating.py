...
def sum_light(els: List[Union[datetime, Tuple[datetime, int]]],
    ...
    if operating != None:
        for i2 in range(max_light):
            ...
            lights[i2][4] = operating.total_seconds()
            ...
            if i3 == lamp_number:
                ...
                if lights[lamp_number][0]:
                    ...
                    if lights[lamp_number][4] > 0:
                        ...
                        lights[lamp_number][4] -= timedelta(lights[lamp_number][2] - lights[lamp_number][1]).total_seconds()
...
#             1.2.1 Проверяем границы наблюдения start_control
#             1.2.2 Проверяем границы наблюдения end_control
#             1.2.3 Производим вычисление времени освещения комнаты
...
#     2. Проверяем время работы лампочек при поступлении любого сигнала
    ...
    #         2.1.2. Если лампочка горела:
        #         2.1.2.1. Проверяем выработку ее ресурса - Если он остался или он бесконечен (не задан):
        #               (т.е. время работы лампочки > 0)
        #           - выключаем лампочку (lights[номер_лампочки][0] = False)
        #           - запоминаем время выключения (lights[номер_лампочки][1])
        #           - делаем рассчет времени работы:
        #
        #                   Если время включения + время работы лампочки < время выключения
        #                       То время выключения = время включения + время работы лампочки
    #                       время работы лампочки = время работы лампочки - (время выключения - время включения)
        #
        #         2.1.2.2.  Проверяем выработку ее ресурса - Если он задан и <= 0: -- данного шага не будет из условия 2.1.2
    #     2.2 Перебираем все лампочки - Иначе сигнал по любой другой лампочки:
    #         2.2.1. Если лампочка горела:
    #             Проверяем выработку ее ресурса (если он задан и > 0)
    #                 Если текущее время сигнала - время включения > время работы лампочки
            #           - выключаем лампочку (lights[номер_лампочки][0] = False)
            #           - запоминаем время выключения (lights[номер_лампочки][1]) = время включения + время работы лампочки
    #                   - время работы лампочки = 0
    #                 Иначе ничего, лампочка может гореть и дальше (ресурс или бесконечен или у него есть запас)
    #         2.2.2. Если лампочка не горела горела: ничего и проверять
    #
    #     Производимшаг 1. Проверяем статус освещения комнаты
...
#
            ...
            # 2 - время выключения
            # 3 - заданное время работы лампочки (ее ресурс)
            # 4 - время работы лампочки (total_secunds)
